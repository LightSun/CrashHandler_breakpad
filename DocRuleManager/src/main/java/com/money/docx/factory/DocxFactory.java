...
public class DocxFactory extends StyleFactory {
    ...
    public static Tbl createTable(DocxTable docxTable, WordprocessingMLPackage wpk) {
        return createTable(docxTable, wpk, null);
    }
    /**
     * 创建表格
     *
     ...
     * @return {@link Tbl}
     */
    public static Tbl createTable(DocxTable docxTable, WordprocessingMLPackage wpk, DocxStyle table_style) {
        ...
        Tbl tbl = TblFactory.createTable(docxTable.getRowCount(), docxTable.getColumnCount(),
                writableWidthTwips / docxTable.getColumnCount());
        //
        if(table_style != null && table_style.getTableIndent() != null){
            TblPr tblPr = factory.createTblPr();
            tbl.setTblPr(tblPr);
            // 3. 创建表格缩进对象 TblInd
            TblWidth tblInd = factory.createTblWidth();
            tblPr.setTblInd(tblInd);
            // 4. 设置缩进类型和值（例如：左缩进 1440 twip，即 1 英寸）
            tblInd.setType("dxa"); // 或者使用 "auto"
            tblInd.setW(BigInteger.valueOf(table_style.getTableIndent()));
        }
        // 调整列宽比例
        adjustTableColWidth(tbl, docxTable.getRows().get(0));
        // 填充表
        if(table_style == null){
            table_style = DocxStyle.builder().build();
        }
        for (int i = 0; i < docxTable.getRowCount(); i++) {
            ...
            for (int j = 0; j < docxTable.getColumnCount(); j++) {
                DocxStyle style = table_style.copy();
