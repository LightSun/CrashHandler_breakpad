...
public class StyleFactory {
    ...
    public static void setIndents(PPr pPr, DocxStyle style) {
        // 设置具体的缩进值（单位是twip，1英寸 = 1440 twip）
        // 设置左缩进为 1英寸 (1440 twip)
        if (style != null) {
            PPrBase.Ind indentation = factory.createPPrBaseInd();
            boolean update = false;
            if(style.getLeftIndent() != null){
                indentation.setLeft(BigInteger.valueOf(style.getLeftIndent()));
                update = true;
            }
            if(style.getRightIndent() != null){
                indentation.setRight(BigInteger.valueOf(style.getRightIndent()));
                update = true;
            }
            if(style.getFirstLineIndent() != null){
                indentation.setFirstLine(BigInteger.valueOf(style.getFirstLineIndent()));
                update = true;
            }
            if(update){
